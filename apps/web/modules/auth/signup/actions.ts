...
export const createUserAction = actionClient.inputSchema(ZCreateUserAction).action(
  withAuditLogging("created", "user", async ({ ctx, parsedInput }) => {
    ...
    // Resolved once and reused by the domain gate and the next-step decision below, so the two cannot
    // drift on what counts as a valid invite. Returns "missing" without any I/O when there is no token.
    ...
    if (outcome.status === "created") {
      const { user } = outcome;
      await handlePostUserCreation(ctx, outcome, parsedInput.inviteToken);
